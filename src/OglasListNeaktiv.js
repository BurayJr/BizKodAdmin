...
function OglasListAktiv() {
    ...
    let getOglasi = async () => {
        try {
            ...
            data?.items.forEach(element => {
                const pendingOglasi = data?.items.filter(oglas => oglas?.status === "inactive");
    ...
    return (
        <>
            ...
            <img src="https://cdn-icons-png.freepik.com/512/9972/9972749.png" width="30" height="30"  />
            <h1 className="h3 mb-0 text-gray-800 text-left ml-2 font-weight-bold ">LISTA NEAKTIVNIH OGLASA</h1>
            ...
            <img src={listOfImages[index % listOfImages.length]} alt={`Picture ${index}`} />
                                                    ...
                                                    <td className = "text-center">{oglas?.user?.firstName}</td>
                                                     ...
                                                     <div class="d-flex justify-content-center align-items-center">
                                                            <button onClick={() => handleAccept(oglas?._id)}  class="btn btn-success btn-sm mr-1 btn-primary w-100 font-weight-bold">PRIHVATI</button>
                                                        </div>
                                                    </th> 
                                                </tr>
                                            )
