    ...
    let getOglasi = async () => {
        try {
            ...
            data?.items.forEach(element => {
                ...
                console.log(element?.status==="pending" ? "da" : "ne");

                const pendingOglasi = data?.items.filter(oglas => oglas?.status === "pending");
    ...
    let handleAccept = async (_id) => {
        try {
            const response = await fetch('http://192.168.0.184:4000/acceptAdvertisment', {
                ...
                body: JSON.stringify({ id: _id, status: 'active' }),
              })
              const data = await response.json();
              console.log(data);
              window.location.reload();
    ...
    return (
        <>
            <div className="d-sm-flex align-items-center justify-content-between mb-4">
                <h1 className="h3 mb-0 text-gray-800 text-left ml-2 font-weight-bold ">LISTA OGLASA NA ČEKANJU</h1>
            ...
            <img src={listOfImages[index % listOfImages.length]} alt={`Picture ${index}`} />
                                                    ...
                                                    <td className = "text-center">{oglas?.user?.firstName}</td>
                                                     ...
                                                     <div className="d-flex justify-content-between align-items-center ">
                                                            <button onClick={() => handleAccept(oglas?._id)}  className="btn btn-success btn-sm mr-1 btn-primary w-50 font-weight-bold mr-1">PRIHVATI</button>
                                                            
                                                            <button onClick={() => handleRefuse(oglas?._id)}  className="btn btn-danger btn-sm mr-1 btn-primary w-50 font-weight-bold ml-1">ODBACI</button>
                                                        </div>
                                                    </th> 
                                                </tr>
                                            )
